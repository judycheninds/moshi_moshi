...
// ==========================================
// AUTH & CRM — In-memory User Store
// ==========================================
const JWT_SECRET = process.env.JWT_SECRET || 'moshi-moshi-secret-2025';
const users = new Map();       // email -> { id, email, passwordHash, name, phone, createdAt }
const userReservations = new Map(); // userId -> [ ...reservation objects ]

// Auth Middleware — attach req.user if valid token
...
app.post('/api/auth/register', async (req, res) => {
    ...
    const user = { id, email: email.toLowerCase(), passwordHash, name, phone: phone || '', createdAt: new Date().toISOString() };
    users.set(email.toLowerCase(), user);
    userReservations.set(id, []);
    const token = jwt.sign({ id, email: user.email, name: user.name }, JWT_SECRET, { expiresIn: '30d' });
    res.json({ success: true, token, user: { id, email: user.email, name: user.name, phone: user.phone } });
});
...
// GET /api/reservations — get logged-in user's reservation history
app.get('/api/reservations', authMiddleware, (req, res) => {
    const history = userReservations.get(req.user.id) || [];
    res.json(history.slice().reverse()); // newest first
});

// Helper to save a completed reservation
function saveReservation(userId, data) {
    if (!userId) return;
    if (!userReservations.has(userId)) userReservations.set(userId, []);
    userReservations.get(userId).push({
        id: `res_${Date.now()}`,
        ...data,
